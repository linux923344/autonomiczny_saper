...
class GraphCreatorBestFS:
    ...
    def getSuccsessorsList(self, v):
        successors = []
        if v.y < 7:
            if self.board.board[v.y+1][v.x] == None or type(self.board.board[v.y+1][v.x]) is Tool:
        ...
        if v.y > 0:
            if self.board.board[v.y-1][v.x] == None or type(self.board.board[v.y-1][v.x]) is Tool:
                ...
                successors.append(successor)
        if v.x < 11:
            if self.board.board[v.y][v.x + 1] == None or type(self.board.board[v.y][v.x + 1]) is Tool:
